...
清华大学电子系第十届人工智能挑战赛（原电子系第 28 届队式程序设计大赛 teamstyle28）

## 赛题背景

## 比赛规则

https://docs.eesast.com/docs/contests/THUAI10   (To be completed)

## 软件架构

![structure](resource/structure_compressed.png)

注：感觉今年不会大变，当然欢迎部员们继续优化或提出新的框架。
...
|   子目录   |                               说明                               |           主要开发组           |
| :--------: | :--------------------------------------------------------------: | :----------------------------: |
|  .github  |              CI，用于选手包同步到服务器上供选手下载              |             平台组             |
|    CAPI    |                     选手接口，生成可执行文件                     |             通信组             |
|    docs    |                          比赛的说明文档                          | 逻辑组、通信组、平台组、界面组 |
| dependency |    项目依赖文件，如 proto、dll、lib、dockerfile、shell 脚本等    | 逻辑组、通信组、平台组、界面组 |
| installer | 下载器兼启动器，用于选手包的下载与更新，生成可执行文件，启动游戏 |             平台组             |
...
- 熟练运用面向对象编程的思想，设计架构时尽可能降低模块与模块的耦合性，保证代码的可维护性
  ...
  - 适当设计单元测试，保证代码的正确运行
- 注意跨平台问题，代码需保证同时支持 Windows 、macOS 与 Linux，避免直接的系统调用带来的跨平台问题
- 注意维护开发文档，便于后来者快速了解本仓库代码结构
- 小组内合理分工，避免个人任务量过重或过轻
- 做好部会记录，及时完成工作任务，避免拖延到ddl，最终发布前应有2周以上的环境搭建与测试时间。
- 各组间多交流，相互了解各自的开发进度，加强协作，遇到困难互相帮助。
- 合理使用 AI ，确保项目一定是可控状态！！！
- 加油，奥里给，THUAI 的中兴之年就靠你们了。
